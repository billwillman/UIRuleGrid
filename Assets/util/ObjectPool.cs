...
public class ObjectPool<T> where T : class, new() {
    ...
    public virtual T GetObject() {
        if (m_objStack!=null&&m_objStack.Count > 0) {
            T t = m_objStack.Pop();
    ...
    protected Stack<T> m_objStack = null;
    protected Action<T> m_resetAction = null;
    private CreateFunc m_createFunc = null;
}

public class MultiObjPool<T> : ObjectPool<T> where T : class, new() {
    public delegate T MultiCreateFunc(int index);

    public void Init(int poolSize, MultiCreateFunc createFunc = null, Action<T> resetAction = null, int index = 0) {
        m_objStack = new Stack<T>();
        m_resetAction = resetAction;
        m_multiCrtFun = createFunc;
        this.m_index = index;
        for (int i = 0; i < poolSize; i++) {
            T item = (m_multiCrtFun != null) ? m_multiCrtFun(m_index) : new T();
            m_objStack.Push(item);
        }
    }
    ...
    public override T GetObject() {
        ...
        return (m_multiCrtFun != null) ? m_multiCrtFun(m_index) : new T();
    }

    MultiCreateFunc m_multiCrtFun;
